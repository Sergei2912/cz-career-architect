...
@app.get("/health")
async def health():
    return {"status": "online", "version": VERSION, "model": os.getenv("OPENAI_MODEL", "gpt-5.2")}
...
async def chat(request: ChatRequest):
    ...
    return ChatResponse(
        response=response, session_id=session_id, suggestions=get_suggestions(request.message)
    )
