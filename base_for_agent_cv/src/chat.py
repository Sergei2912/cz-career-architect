...
class ChatAssistant:
    def __init__(self):
        ...
        self._check_gdpr = check_gdpr
        self._check_csn = check_csn_typography
        self.model = get_model_name()
        self.agent = create_agent(
            "CZ Career Architect v1.2.3",
            CHAT_SYSTEM_PROMPT,
            self.model
        )
    ...
    def validate_text(self, text):
        findings = []
        findings.extend(self._check_gdpr(text))
        findings.extend(self._check_csn(text, check_nbsp=False))
        return findings
